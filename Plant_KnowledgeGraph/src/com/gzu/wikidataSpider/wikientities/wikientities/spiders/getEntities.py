...
class entitiesSpider(scrapy.spiders.Spider):
	...
	def parse(self,response):
		...
		jsonItemList = list()
		entityCount = 0
		with open('E:\学习\毕业论文文献及项目\项目\Plant_KnowledgeGraph\src\com\gzu\wikidataSpider\wikientities\iplant_entityMark.txt','r',encoding='utf8') as f:
			for line in f:
				entity = line.split(" ")[0]
				if(len(line.split(" ")) >= 2):
		...
		for url in url_list:
			...
			entityjson = httpRequest.get(url,headers=headers,timeout=10).json()
			httpRequest.close()
			entityNumber = str()
