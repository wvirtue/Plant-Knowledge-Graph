...
class Model:
    ...
    def _build_net(self):
        ...
        # # a = tf.transpose(bilstm_at,[0, 2, 1])
        # BILSTM
        bilstm_out = tf.tanh(tf.matmul(bilstm_out, W) + b)

        #BILSTM+ATT
        # bilstm_out = tf.tanh(bilstm_out)
        # bilstm_at = tf.nn.softmax(bilstm_out)
        # bilstm_att = tf.matmul(bilstm_at,W) + b
        ...
        #att+CRF
        # log_likelihood, self.transition_params = tf.contrib.crf.crf_log_likelihood(bilstm_att, self.labels, tf.tile(np.array([self.sen_len]),np.array([self.batch_size])))
        ...
        #ATT+CRF
        # self.viterbi_sequence, viterbi_score = tf.contrib.crf.crf_decode(bilstm_att, self.transition_params,tf.tile(np.array([self.sen_len]),np.array([self.batch_size])))
