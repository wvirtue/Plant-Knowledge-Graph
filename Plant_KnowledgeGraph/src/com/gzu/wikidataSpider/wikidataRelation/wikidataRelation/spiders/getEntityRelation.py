...
class entityRelationSpider(scrapy.spiders.Spider):
	...
	def parse(self, response):
		...
		filePath = "E:\学习\毕业论文文献及项目\项目\Plant_KnowledgeGraph\src\com\gzu\wikidataSpider\wikidataRelation\data"
		#获取已经爬取的数据(避免重复爬)
		alreadyGet = []
		...
		with open(filePath+"\\relationResult.json", "r",encoding = 'utf-8') as fr:
		...
		count = 0 
		with open(filePath+"./readytoCrawl.json","r",encoding = 'utf-8') as fr:
			for line in fr.readlines():
				count += 1 
				# print(1.0*count/17500)
					...
					for relatedEntity in relationItem.xpath('.//div[contains(@class,"wikibase-statementview-mainsnak")]//div[contains(@class,"wikibase-statementview-mainsnak")]\
						...
						//div[contains(@class,"wikibase-snakview-value")]//a[contains(@title,"Q")]'):
							...
							if(len(entityId) == 0):
								...
								relatedEntityJson = httpRequest.get(url,headers=headers,timeout=20).json()
								httpRequest.close()
								entity2 = str()
