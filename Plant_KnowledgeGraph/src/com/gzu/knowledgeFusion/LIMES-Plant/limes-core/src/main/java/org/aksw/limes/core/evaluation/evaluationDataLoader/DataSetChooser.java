...
public class DataSetChooser {
    ...
    private static HashMap<MapKey, Object> getPlantMesh() {
        HashMap<MapKey, Object> param = new HashMap<MapKey, Object>();
        // folders & files
        param.put(MapKey.BASE_FOLDER, "src/main/resources/datasets/"/* "Examples/GeneticEval/" */);
        param.put(MapKey.DATASET_FOLDER, "src/main/resources/datasets/plant/" /* "Datasets/Abt-Buy/" */);
        param.put(MapKey.CONFIG_FILE, "config.xml");
        param.put(MapKey.REFERENCE_FILE, "plant_perfectMapping.csv");
        param.put(MapKey.SOURCE_FILE, "plant_source.csv");
        param.put(MapKey.TARGET_FILE, "plant_target.csv");

        param.put(MapKey.EVALUATION_RESULTS_FOLDER, "resources/results/");
        param.put(MapKey.EVALUATION_FILENAME, "Pseudo_eval_plant.csv");
        param.put(MapKey.NAME, "plant");
        // data
        AConfigurationReader cR = new XMLConfigurationReader(
                (String) param.get(MapKey.BASE_FOLDER) + param.get(MapKey.CONFIG_FILE));
        cR.read();
        ...
        param.put(MapKey.REFERENCE_MAPPING,
                OracleFactory
                        .getOracle((String) /* param.get(MapKey.BASE_FOLDER)+ */param.get(MapKey.DATASET_FOLDER)
                                + param.get(MapKey.REFERENCE_FILE), "csv", "simple")
                        .getMapping());
        param.put(MapKey.SOURCE_CLASS, "baidu:product");
        param.put(MapKey.TARGET_CLASS, "wiki:product");
        return param;
    }
    ...
    public enum DataSets {
        PERSON1, PERSON1_CSV, PERSON2, PERSON2_CSV, RESTAURANTS, OAEI2014BOOKS, RESTAURANTS_FIXED, DBLPACM, ABTBUY,PLANTMESH, DBLPSCHOLAR, AMAZONGOOGLEPRODUCTS, DBPLINKEDMDB, DRUGS, RESTAURANTS_CSV// ,TOWNS,
        // VILLAGES,
        // MOVIES
    }
